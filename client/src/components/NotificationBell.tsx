...
export function NotificationBell() {
  ...
  const getToastIcon = (title: string, type: string) => {
    ...
    if (
      ...
      titleStr.includes("unauthorized")
    ) {
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        ...
        >
          <Bell
            className={`h-5 w-5 ${unreadCount > 0 ? "text-white" : "text-foreground"}`}
          />
          {unreadCount > 0 && (
            <Badge className="absolute -top-2 -right-2 h-5 w-5 flex items-center justify-center p-0 bg-red-500 text-white text-xs font-bold rounded-full animate-pulse">
      ...
      <PopoverContent
        className="w-80 p-0 shadow-2xl border rounded-lg z-50"
        align="end"
      >
            ...
            <div className="space-y-2 p-2">
              {notifications.map((notification) => {
                const borderLeftColor =
                  notification.title.toLowerCase().includes("entering") ||
                  notification.title.toLowerCase().includes("exiting")
                    ? "border-l-green-500"
